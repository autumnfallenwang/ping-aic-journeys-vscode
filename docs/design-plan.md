...
### D46 — Realm-level journey export + import (M9 Phase 5)

**Extends D42/D45 to realm scope.** One action exports **every journey in a realm** to a single frodo/PAIC-UI-compatible bundle; the existing Transfer page imports it unchanged. Validated empirically against the `poc/onprem-am/` bed (AM 7.5.2) and against a captured 23-tree customer export. Locked sub-decisions:

- **Envelope: the SAME `{ meta, trees }` bundle — no new format, no folder.** A realm export is `listTrees(realm)` fed through the existing per-tree assembler. Prior art agrees: frodo's `exportJourneys` (`ref/frodo-lib/src/ops/JourneyOps.ts:1427`) loops every tree in the realm into one `MultiTreeExportInterface` and `delete`s each tree's own `meta` (one `meta` per file, never per tree); the PAIC-UI "Export Journeys" dialog does the same from a multi-select. Filename follows frodo: **`all<Realm>Journeys.journey.json`** — `getRealmString()` reproduced exactly (split on `/`, `titleCase` each segment, concatenate), so `alpha` → `allAlphaJourneys`, `alpha/customers` → `allAlphaCustomersJourneys`, and the root realm collapses to `allJourneys`. **Deliberately plain, matching the single-journey `<JourneyName>.journey.json`.** The PAIC UI instead names every export `<journey>-journeyExport-<realm>-realm-<host>-<ISO timestamp>.json` (its captures confirm the timestamp is on single-journey exports too — two exports of one journey 77s apart differ only by it). We do **not** copy that: `meta.origin` + `meta.exportDate` already carry provenance and the Transfer page surfaces them, so putting host/date in the name duplicates the file's own metadata and would leave the tool with two naming conventions. Accepted trade-off: exporting the same realm from two tenants yields the same default name — the save dialog is where that gets resolved.
- **No depth toggle at realm scope.** A realm's tree listing already includes its `innerTreeOnly` trees and `InnerTreeEvaluatorNode` refs resolve within the realm, so "every journey in the realm" **is** the closure — `level1` is meaningless here. Stamp `depthMode:"allLevels"` + informational `scope:"realm"`. **The toggle stays for single-journey export** (D42/TD-5), which is unchanged.
- **Leaf duplication stays in the FILE, is removed from the FETCH.** Each tree carries its own copy of the leaves it references (frodo + PAIC-UI both do this; a shared top-level `scripts` map would be our own dialect and would break `frodo journey import` / the PAIC-UI Import). Measured on the captured 23-tree export: 248 script entries / 156 unique = **29.5% of a 3.82 MB file is duplicated script bodies** — accepted for interop. But `seenScripts` (`src/export/journey-bundle.ts:118`) is per-`assembleTree`, so a shared library is re-fetched once per referencing tree; **hoist that cache to the realm builder**. The import already dedupes on the way in by `kind:id` (PD-6), so no double-writes.
- **Rejected: one file per journey.** Tested (`frodo -A` shape, 6 files from the bed's alpha realm). Splitting **destroys the information the gate depends on**: `discover.ts:106` computes `innerJourneys = referenced && !bundled`, so every inner ref in a single-tree file becomes a missing prerequisite at `severity:"blocking"` — 2 of 3 test files were hard-blocked on a fresh target. It also forfeits `topoOrder` (the user would have to know the write order), PD-6 dedupe, and one freeze/drift window instead of N. It isn't even cheaper to ship: `panel.ts` opens bundles with `canSelectMany:false`, so N files = N full import runs or a new multi-file flow. Separate files remain a plausible **future** "config-as-code" export (the fr-config-manager model, which also extracts script bodies to real `.js` files) — a different feature, not a transfer bundle.
- **Import: extend the existing Transfer page. No new surface, no mode selector.** `parseBundle` already branches on `isRecord(root.trees)`; a 1-tree and a 23-tree bundle take the identical path. Verified live against the bed: a 6-tree realm bundle produced a correct flat plan vs another realm with **zero code changes** — cross-tree leaf dedupe, subject/inner role derivation, gates, and executor ordering all held.
- **Roles stay reference-derived; a realm bundle yields MANY subjects and MANY inners.** Confirmed on the bed: `OnPremLogin`/`AlphaLogin`/`OnPremOtpOuter` = subject, `OnPremMfaInner`/`AlphaMfaInner`/`OnPremEmailOtp` = inner. Note **role ≠ importance** — `OnPremEmailOtp` is a genuine standalone login journey but is classified `inner` (and so defaults to Keep) purely because another bundled tree references it.
...
**Empirically retracted — sub-realms do NOT inherit parent trees.** An earlier reading of this work claimed AM sub-realms inherit the parent realm's journeys, which would have made a realm export sweep in borrowed trees and a realm import shadow them. **That was wrong.** AM **copies the parent's trees into a sub-realm at realm-creation time**, as real independent local trees. Proof on the bed: a root-only tree (`PropProbe`) 404s from `alpha`; a freshly-created realm lists root's 5 trees; but a tree added to root *after* that realm existed is never picked up by it (`GET delta/trees/TempProbe` → 404). The original 200 that suggested inheritance was on a name the sub-realm owned outright. **Consequence: nothing to build.** A realm export exports that realm's own trees; import cannot shadow a parent. The only residue is benign — a newly-created sub-realm is not empty, so its inherited-at-creation copies show as `exists` rather than `new`.

**Scope honesty.** This is an **all-journeys** sweep, not a realm backup: leaves travel only if some journey references them, so orphan scripts (the Search page's "orphans" view), unreferenced themes, and unreferenced ESVs are **not** included. The button reads just **"Export…"**, matching `JourneyCard`'s, so both export affordances look the same — the realm card's context makes the target obvious. Never call it "backup".

**Known-untested at scale.** Every live proof here was 8 components / ~150 ms on-prem. The real target is ~180 components and ~500 writes. On-prem also has `caps.themes:false` / no email templates / no ESVs, so **three PAIC-only leaf kinds were never exercised** — most notably `client.ts:573` `writeTheme`, which does a full GET + PUT of the entire `ui/themerealm` document **per theme**, sequentially with `If-Match` + 412 retry. Correct by construction, but it is O(n) whole-document rewrites and has never run at n > 1 in one import.
