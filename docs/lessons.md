...
## 2026-08-21 — A 200 on a name the child realm owns proves nothing about inheritance (AM sub-realm trees)

**Context:** Scoping realm-level export/import (D46). I needed to know whether an AM sub-realm *inherits* its parent's authentication trees — if it did, a realm export would sweep in borrowed trees and a realm import could shadow the parent's copy.
**Mistake (asserted as a finding, ranked as the #1 design risk):** I ran `GET /realms/root/realms/alpha/…/trees/OnPremLogin` → **200**, saw that the seed script writes `OnPremLogin` only to **root**, and concluded alpha inherits it. Wrong: `alpha` has its **own** `OnPremLogin` (same name, separate object), so the 200 says nothing about inheritance. I picked a probe whose result is identical under both hypotheses. It survived a whole design discussion and shipped into a doc before I re-checked.
**Correction:** Probe with a name the child provably does **not** own. `PropProbe` exists only in root → `GET alpha/…/trees/PropProbe` → **404**, so alpha does *not* inherit. Then a fresh realm listed root's 5 trees, which looked like inheritance again — resolved by a **temporal** probe: create realm `delta`, *then* add `TempProbe` to root, then read delta → **404**. Actual behaviour: **AM copies the parent's trees into a sub-realm at realm-creation time, as independent local trees.** Nothing is inherited live; nothing to filter on export; no shadowing on import. (Residue: a "fresh" sub-realm is not empty, so its creation-time copies show as `exists`, not `new`.)
**How to avoid next time:** Before treating a probe as evidence, ask **"what would this return under the opposite hypothesis?"** — if the answer is "the same thing," it isn't a test. For inheritance/fallback semantics specifically: (1) probe a name that exists **only** in the parent, never one both may own; (2) add a **temporal** probe (mutate the parent *after* the child exists) to separate live inheritance from a creation-time copy. Cheap to run against `poc/onprem-am/`; expensive to unwind after it's written into a design doc as a locked risk.

## 2026-08-21 — Check for a supersession in the CODE before reporting a doc/code divergence

**Context:** Same session — I flagged that the plan table seeds writable rows **checked** (`App.tsx:298-304`) while D42/TD-10 documents *"default OFF, opt-in row by row"*, and raised it to the user as a blocking open question.
**Mistake:** There was no divergence. The comment **three lines above the code I quoted** reads *"Smart-default selection (S9a, refines TD-10)"* — a later decision had deliberately superseded TD-10, and `docs/progress.md` already described the shipped behaviour as "smart-default selection". I read the design doc as current and the code as suspect, when the code carried its own provenance.
**Correction:** S9a is the rule; D42's TD-10 sentence was stale. Annotated the TD-10 line at its source with a strike-through pointing to S9a + D46, so the next top-to-bottom reader of D42 doesn't inherit the same wrong conclusion.
**Update (2026-08-22):** the underlying rule has since changed again — **D47** makes an overwrite opt-in, so
a `differs` leaf is default-OFF once more (creates and the main journey still seed checked). The lesson below
is unaffected — it is about *how* to check for supersession, not about which way this particular default
points. Read the chain as TD-10 → S9a (D46) → D47.
**How to avoid next time:** A locked decision in `design-plan.md` is a snapshot, not a guarantee of currency — slice-level refinements (S-numbers, PD-numbers) often supersede it and are recorded **in the code comment** or in `progress.md`, not always back-propagated. Before reporting "doc says X, code does Y": read the **full comment block** around the code (not just the quoted lines), grep `progress.md` for the shipped-behaviour wording, and search the doc for a later refinement id. If the code cites a decision id the doc paragraph doesn't mention, the code is the newer record — and fix the stale doc line at its source rather than only noting it downstream.
